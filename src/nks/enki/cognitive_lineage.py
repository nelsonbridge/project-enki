"""Governed cognitive-lineage profile for Enki temporal graphs.
...
class CognitiveStance(StrEnum):
    """Local reasoning state; never a substitute for Enki authority."""
...
class CognitiveLineageSnapshot(BaseModel):
    """Non-inferential view of what cognitive records were known at a coordinate."""
...
class CognitiveLineage:
    ...
    def snapshot(self, *, as_of: datetime, known_at: datetime) -> CognitiveLineageSnapshot:
        """Return recorded stances only; do not infer a synthesized current belief."""
        ...
        known_ids = set(temporal.known_record_ids)
