...
def _entry(
    ...
    *,
    ...
    context_ids: tuple[str, ...] = ("ctx:enki-design",),
    ...
    return CognitiveLineageEntry(
        ...
        node_type=node_type,
        stance=stance,
        domain_id="domain:enki-architecture",
...
def test_subject_acceptance_still_does_not_manufacture_enki_authority() -> None:
...
def test_abandoned_hypothesis_remains_historical_record() -> None:
    ...
    assert "idea:abandoned" in lineage.store.query(as_of=T1, known_at=T1).known_record_ids
...
def test_bounded_real_design_lineage_can_represent_reasoning_chain() -> None:
    ...
    links = [
        _link("edge:obs-q", observation.entry_id, question.entry_id, TemporalGraphEdgeType.INFORMS),
        _link("edge:q-h", question.entry_id, hypothesis.entry_id, TemporalGraphEdgeType.INFORMS, recorded_at=T1),
        _link("edge:e-h", evidence.entry_id, hypothesis.entry_id, TemporalGraphEdgeType.SUPPORTS, recorded_at=T1),
        _link("edge:h-i", hypothesis.entry_id, interpretation.entry_id, TemporalGraphEdgeType.INFORMS, recorded_at=T2),
        _link("edge:i-d", interpretation.entry_id, decision.entry_id, TemporalGraphEdgeType.DECIDED_FROM, recorded_at=T2),
        _link("edge:d-o", decision.entry_id, outcome.entry_id, TemporalGraphEdgeType.RESULTED_IN, recorded_at=T3),
    ]

    lineage = CognitiveLineage(
        [observation, question, hypothesis, evidence, interpretation, decision, outcome],
        links,
    )

    snapshot = lineage.snapshot(as_of=T3, known_at=T3)
    assert snapshot.entry_ids_by_stance["suspected"] == ("cog:h:federated-grammar",)
    assert snapshot.entry_ids_by_stance["qualified"] == ("cog:i:federate-not-collapse",)
...
def test_same_as_of_can_differ_by_known_at_without_rewriting_thought_history() -> None:
    ...
    before_enki_knew = lineage.snapshot(as_of=T1, known_at=T1)
    after_enki_knew = lineage.snapshot(as_of=T1, known_at=T3)

    assert "idea:retroactive-revision" not in {
        item for ids in before_enki_knew.entry_ids_by_stance.values() for item in ids
    }
    assert after_enki_knew.entry_ids_by_stance["qualified"] == (
        "idea:retroactive-revision",
    )
